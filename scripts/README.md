...
叠加 `AIO_MESSAGE_BROWSER_ONLY=1` 验证问候 0 tokens、真实 Compose 行内引用、伪造引用不可点击及条目打开；截图为 `test-results/message-{desktop,mobile}*.png`。服务回归 `memory-routing-tests.mjs` 同时验证问候不创建队列任务、不进入图谱、重复请求不增加消息，普通资料和凭据仍正常整理。先按 Memory README 重建开发运行器，避免旧宿主二进制与当前 Component 不一致。
# v2 Process
