...
配对、任务通道、工具实现、智能体调度是四个独立层次。设备在线只证明通道可用。当前设备端声明的 `desktop.open-app` 可启动应用并回报实际 PID；该启动能力本身不提供桌面编辑。新增 `desktop.control` 经本机单独授权后提供桌面观察与输入，接入方式与实测限制见 [桌面控制](desktop-control.md)。Agent 目前是中心协调器，配对客户端是工具执行 worker；尚未实现每台设备独立运行模型的蜂群。
