...
先 get_app_state 获取 app 对应的 observation，再调用 create_spreadsheet，arguments 示例：`{"app":"com.kingsoft.wpsoffice.mac","filename":"小明年龄表.xlsx","sheet_name":"人员信息","rows":[["姓名","年龄"],["小明",18]]}`。worker 只接受标量单元格和等宽行，不接受公式对象、宏或任意路径。每次在本机私有状态目录 documents 下创建独立子目录，因此同名文件不会覆盖。
